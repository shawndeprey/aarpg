...
func _on_visibility_changed() -> void:
	...
	if visible == true:
		...
		for q in QuestManager.current_quests:
			...
			if quest_data == null: continue
			var new_q_item: QuestItem = QUEST_ITEM.instantiate() as QuestItem
...
func update_quest_details(q: Quest) -> void:
	...
	for step in q.steps:
		var new_step: QuestStepItem = QUEST_STEP_ITEM.instantiate() as QuestStepItem
		var step_is_complete: bool = false
		if quest_save.title != "not_found":
