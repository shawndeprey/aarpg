class_name DarkWizardBoss extends Node
...
@onready var persistent_data_handler: PersistentDataHandeler = $PersistentDataHandler
...
func damage_taken(hb: HurtBox) -> void:
	if hp < 1 or animation_player_damaged.current_animation == "damaged" or hb.damage == 0:
		return
	hp = clampi(hp - hb.damage, 0, max_hp)
	damage_count += 1
...
func explosion(_p: Vector2 = Vector2.ZERO) -> void:
	var e: Node2D = ENERGY_EXPLOSION_SCENE.instantiate() as Node2D
